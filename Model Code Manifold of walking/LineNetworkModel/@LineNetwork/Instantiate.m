...
%%  Projection biases (Gaussians)
    ...
    Sp1ProjIx = DistDomain(int32(length(DistDomain)*obj.Means(1)));
    Sp2ProjIx = DistDomain(int32(length(DistDomain)*obj.Means(2)));
    Sp3ProjIx = DistDomain(int32(length(DistDomain)*obj.Means(3)));
    Sp4ProjIx = DistDomain(int32(length(DistDomain)*obj.Means(4)));
    Sp5ProjIx = DistDomain(int32(length(DistDomain)*obj.Means(5)));

    
    PDF1 = exp(-(DistDomain - Sp1ProjIx).^2 / (2 * obj.Vars(1)^2));
    ...
    PDF2 = exp(-(DistDomain - Sp2ProjIx).^2 / (2 * obj.Vars(2)^2));
    PDF2 = [0 PDF2(1:end-1)] + PDF2;
    PDF2 = PDF2/sum(PDF2);

    PDF3 = exp(-(DistDomain - Sp3ProjIx).^2 / (2 * obj.Vars(3)^2));
    PDF3 = [0 PDF3(1:end-1)] + PDF3;
    PDF3 = PDF3/sum(PDF3);

    PDF4 = exp(-(DistDomain - Sp4ProjIx).^2 / (2 * obj.Vars(4)^2));
    PDF4 = [0 PDF4(1:end-1)] + PDF4;
    PDF4 = PDF4/sum(PDF4);

    PDF5 = exp(-(DistDomain - Sp5ProjIx).^2 / (2 * obj.Vars(5)^2));
    PDF5 = [0 PDF5(1:end-1)] + PDF5;
    PDF5 = PDF5/sum(PDF5);

%% Connection propabilies

    Prob1 = 30*PDF1(int32(Y1));
    Prob1(Prob1 > 1) = 1;

    Prob2 = 30*PDF2(int32(Y2));
    Prob2(Prob2 > 1) = 1;

    Prob3 = 30*PDF3(int32(Y3));
    Prob3(Prob3 > 1) = 1;

    Prob4 = 30*PDF4(int32(Y4));
    Prob4(Prob4 > 1) = 1;

    Prob5 = 30*PDF5(int32(Y5));
    Prob5(Prob5 > 1) = 1;
